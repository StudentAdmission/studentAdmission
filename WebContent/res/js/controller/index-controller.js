...
app.controller('indexCtrl', ['$http', '$scope', 'loginService', 'introduceService', 'reportingService', 'validateService',
    function ($http, $scope, loginService, introduceService, reportingService, validateService) {
        ...
        $scope.saLogin = function () {
            if ($scope.login.loginNum === "") {
                ...
                $scope.login.loginPwd = md5($scope.login.loginPwd);  //对密码进行加密
                $http.post('login.do', $scope.login).then(function (response) {  //向后台查询登录信息
                    $scope.login.loginPwd = "";
                    var responseData = response.data;
                    if (responseData.status === 0) {
                        toastr.error('用户名/密码输入有误');
                        ...
                        $http.post('login/time.do', {
                            ...
                            if (response.data.status === 1) {
                                ...
                                toastr.success('登录成功');
                                $http.post('getLogin.do', "'" + loginService.getLoginSession() + "'").then(function (response) {
                                    $('#personalCenter>img').attr('src', response.data.data.loginPortrait ? (AVATAR_PATH + response.data.data.loginPortrait) : 'res/img/avatar.jpg');
                                });
                            }
                        }, function () {
        ...
        $scope.submitNewPwd = function () {
            ...
            if (forgetTag.loginNum && forgetTag.loginEmail && forgetTag.loginCode && forgetTag.loginPwd) {
                ...
                if (forgotten.num === '') {
                    ...
                    $http.post('forget/pwd.do', "{loginNum:'" + forgotten.num + "',loginEmail:'" + forgotten.email + "',loginPwd:'" + forgotten.newPwd + "'}")
                        .then(function (response) {
                            if (response.data.status === 1) {
                                toastr.remove();
        ...
        $(function () {
            ...
            setInterval(function () {
                var session = loginService.getLoginSession() ? loginService.getLoginSession() : '';
                if (session) {
